...
The task is not as simple as it seems. This task can be formulated as a problem of data imputation in which the color values of *a and *b values are "hallucinated".
...
%----------------------------- Dataset -----------------------------%
\subsection{Dataset}
Our training, validation and test datasets are picked from the validation set pof MIT's Places 365. The dataset is from the CSAILVision Lab and can be accessed at \cite{1}.
...
The code and instructions to run it are available at \url{github.com/nsudhanva/image-colorization}. 
